...
//-- mod : categories hierarchy ------------------------------------------------
//-- add
			$template->set_switch('attachrow.light', !($i % 2));
...
{
	$pagination = generate_pagination(, $total_rows, $board_config['topics_per_page'], $start).'&nbsp;';
...
//-- add
// pagination
if ( !isset($lang['Files_count']) )
{
	$lang['Files_count'] = ''; // '[%d Files]';
	$lang['Files_count_1'] = ''; // '[%d Files]';
}
$pagination = new pagination('uacp', array('sid' => $user->data['session_id'], 'mode' => $mode, 'order' => $sort_order, POST_USERS_URL => $profiledata['user_id']));
$pagination->display('pagination', $total_rows, $config->data['topics_per_page'], $start, true, 'Files_count');

// navigation
$navigation = new navigation();
$navigation->add($profiledata['username'], '', 'profile', array('mode' => 'viewprofile', POST_USERS_URL => $profiledata['user_id']), '');
$navigation->add('UACP', '', 'uacp', array('sid' => $user->data['session_id'], POST_USERS_URL => $profiledata['user_id']), '');
$navigation->display();
//-- fin mod : categories hierarchy --------------------------------------------
