...
// here we added
//	, privmsgs_sub_title
//	, '" . str_replace("\'", "''", addslashes($privmsg['privmsgs_sub_title'])) . "'
//-- modify
		$sql = "INSERT $sql_priority INTO " . PRIVMSGS_TABLE . " (privmsgs_type, privmsgs_sub_title, privmsgs_subject, privmsgs_from_userid, privmsgs_to_userid, privmsgs_date, privmsgs_ip, privmsgs_enable_html, privmsgs_enable_bbcode, privmsgs_enable_smilies, privmsgs_attach_sig)
			VALUES (" . PRIVMSGS_SENT_MAIL . ", '" . str_replace("\'", "''", addslashes($privmsg['privmsgs_sub_title'])) . "', '" . str_replace("\'", "''", addslashes($privmsg['privmsgs_subject'])) . "', " . $privmsg['privmsgs_from_userid'] . ", " . $privmsg['privmsgs_to_userid'] . ", " . $privmsg['privmsgs_date'] . ", '" . $privmsg['privmsgs_ip'] . "', " . $privmsg['privmsgs_enable_html'] . ", " . $privmsg['privmsgs_enable_bbcode'] . ", " . $privmsg['privmsgs_enable_smilies'] . ", " .  $privmsg['privmsgs_attach_sig'] . ")";
//-- fin mod : PM sub-titles ---------------------------------------------------
		if ( !$db->sql_query($sql) )
		{
...
//-- add
	if ( $sub_title_display )
	{
		$post_sub_title = $privmsg['privmsgs_sub_title'];
	}
//-- fin mod : PM sub-titles ---------------------------------------------------
...
//-- mod : PM sub-titles -------------------------------------------------------
//-- add
		'POST_SUB_TITLE' => ($sub_title_display && !empty($post_sub_title)) ? '<br />' . _censor($post_sub_title) : '',
//-- fin mod : PM sub-titles ---------------------------------------------------
...
//-- add
		if ( $sub_title_display )
		{
			$privmsg_sub_title = trim(strip_tags($HTTP_POST_VARS['sub_title']));
		}
//-- fin mod : PM sub-titles ---------------------------------------------------
...
// here we added
//	, privmsgs_sub_title
//	, '" . str_replace("\'", "''", $privmsg_sub_title) . "'
//-- modify
			$sql_info = "INSERT INTO " . PRIVMSGS_TABLE . " (privmsgs_type, privmsgs_sub_title, privmsgs_subject, privmsgs_from_userid, privmsgs_to_userid, privmsgs_date, privmsgs_ip, privmsgs_enable_html, privmsgs_enable_bbcode, privmsgs_enable_smilies, privmsgs_attach_sig)
				VALUES (" . PRIVMSGS_NEW_MAIL . ", '" . str_replace("\'", "''", $privmsg_sub_title) . "', '" . str_replace("\'", "''", $privmsg_subject) . "', " . $userdata['user_id'] . ", " . $to_userdata['user_id'] . ", $msg_time, '$user_ip', $html_on, $bbcode_on, $smilies_on, $attach_sig)";
//-- fin mod : PM sub-titles ---------------------------------------------------
		}
		else
		{
//-- mod : PM sub-titles -------------------------------------------------------
// here we added
//	, privmsgs_sub_title = '" . str_replace("\'", "''", $privmsg_sub_title) . "'
//-- modify
			$sql_info = "UPDATE " . PRIVMSGS_TABLE . "
				SET privmsgs_type = " . PRIVMSGS_NEW_MAIL . ", privmsgs_sub_title = '" . str_replace("\'", "''", $privmsg_sub_title) . "', privmsgs_subject = '" . str_replace("\'", "''", $privmsg_subject) . "', privmsgs_from_userid = " . $userdata['user_id'] . ", privmsgs_to_userid = " . $to_userdata['user_id'] . ", privmsgs_date = $msg_time, privmsgs_ip = '$user_ip', privmsgs_enable_html = $html_on, privmsgs_enable_bbcode = $bbcode_on, privmsgs_enable_smilies = $smilies_on, privmsgs_attach_sig = $attach_sig 
...
//-- add
		if ( $sub_title_display )
		{
			$privmsg_sub_title = ( isset($HTTP_POST_VARS['sub_title']) ) ? trim(strip_tags(stripslashes($HTTP_POST_VARS['sub_title']))) : '';
		}
//-- fin mod : PM sub-titles ---------------------------------------------------
...
//-- add
			if ( $sub_title_display )
			{
				$privmsg_sub_title = $privmsg['privmsgs_sub_title'];
			}
//-- fin mod : PM sub-titles ---------------------------------------------------
...
//-- add
			if ( $sub_title_display )
			{
				$sql = str_replace('SELECT ', 'SELECT pm.privmsgs_sub_title, ', $sql);
			}
//-- fin mod : PM sub-titles ---------------------------------------------------
			if ( !($result = $db->sql_query($sql)) )
			{
...
//-- mod : PM sub-titles -------------------------------------------------------
//-- add
			'POST_SUB_TITLE' => ($sub_title_display && !empty($privmsg_sub_title)) ? '<br />' . _censor($privmsg_sub_title) : '',
//-- fin mod : PM sub-titles ---------------------------------------------------
...
//-- mod : PM sub-titles -------------------------------------------------------
//-- add
		'SUB_TITLE' => $sub_title_display ? str_replace('"', '&quot;', preg_replace($html_entities_match, $html_entities_replace, $privmsg_sub_title)) : '',
//-- fin mod : PM sub-titles ---------------------------------------------------
...
//-- mod : PM sub-titles -------------------------------------------------------
//-- add
		$msg_sub_title = ($sub_title_display && !empty($row['privmsgs_sub_title'])) ? '<br />&nbsp;' . sprintf($user->lang('Sub_title_desc'), _censor($row['privmsgs_sub_title'])) : '';
//-- fin mod : PM sub-titles ---------------------------------------------------
		if ( count($orig_word) )
		{
...
//-- mod : PM sub-titles -------------------------------------------------------
//-- add
			'SUB_TITLE' => $sub_title_display ? $msg_sub_title : '',
//-- fin mod : PM sub-titles ---------------------------------------------------
			'DATE' => $msg_date,
//-- mod : attachmod -----------------------------------------------------------
//-- add
			'PRIVMSG_ATTACHMENTS_IMG' => defined('CH_mod_attachmod') ? privmsgs_attachment_image($privmsg_id) : '',
//-- fin mod : attachmod -------------------------------------------------------
