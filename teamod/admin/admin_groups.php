...
//-- modify
			$sql = "UPDATE " . GROUPS_TABLE . "
				SET group_type = $group_type, group_status = " . intval($group_status) . ", group_name = '" . str_replace("\'", "''", $group_name) . "', group_description = '" . str_replace("\'", "''", $group_description) . "', group_moderator = $group_moderator 
				WHERE group_id = $group_id";
//-- fin mod : categories hierarchy --------------------------------------------
			if ( !$db->sql_query($sql) )
			{
...
//	, " . intval($group_status) . "
//-- modify
			$sql = "INSERT INTO " . GROUPS_TABLE . " (group_type, group_status, group_name, group_description, group_moderator, group_single_user) 
				VALUES ($group_type, " . intval($group_status) . ", '" . str_replace("\'", "''", $group_name) . "', '" . str_replace("\'", "''", $group_description) . "', $group_moderator,	'0')";
//-- fin mod : categories hierarchy --------------------------------------------
			if ( !$db->sql_query($sql) )
			{
