...
//-- mod : categories hierarchy ------------------------------------------------
//-- add
		$template->set_filenames(array('attachbox' => 'viewtopic_attach_body.tpl'));
		if ( method_exists($template, 'unset_block_vars') )
		{
			$template->assign_lastblock_vars('postrow', array(
				'ATTACHMENTS' => $template->get_pparse('attachbox'),
			));
			$template->unset_block_vars('postrow.attach');
		}
		else
		{
			$add_dot = defined('CH_extended_templates') ? '' : '.';
			$template->_tpldata['postrow' . $add_dot][ count($template->_tpldata['postrow' . $add_dot]) -1 ]['ATTACHMENTS'] = $template->get_pparse('attachbox');
			unset($template->_tpldata['postrow' . $add_dot][ count($template->_tpldata['postrow' . $add_dot]) -1 ]['attach' . $add_dot]);
		}
//-- fin mod : categories hierarchy --------------------------------------------
	}
}
