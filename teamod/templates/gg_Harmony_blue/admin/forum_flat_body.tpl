...
<!--
	function branch_set(varname, from_id, to_id)
	{
		from_found = false;
		index = -1;
		for ( i = 0; i < document.post.elements.length; i++ )
		{
			if ( !from_found )
			{
				if ( document.post.elements[i].name == varname + '[' + from_id + ']' )
				{
					from_found = true;
					index = document.post.elements[i].options.selectedIndex;
					if ( (index == 0) && (varname == 'presets'))
					{
						break;
					}
				}
			...
			if ( from_found )
			{
				...
				{
					document.post.elements[i].options.selectedIndex = index;
					if ( document.post.elements[i].name == varname + '[' + to_id + ']' )
					{
						break;
					}
				}
...
<tr>
	...
	<tr>
		<td class="<!-- BEGIN light -->row1<!-- BEGINELSE light -->row2<!-- END light -->" align="center" height="25" nowrap="nowrap"><span class="gensmall">&nbsp;
			<!-- BEGIN show_propag -->
			<a href="javascript:branch_set('{PROPAG_VAR}', {row.FORUM_ID}, {row.LAST_CHILD_ID});"><img src="{I_COPY}" border="0" align="top" alt="{L_COPY}" title="{L_COPY}" /></a>&nbsp;
			<select name="{PROPAG_VAR}[{row.FORUM_ID}]">{row.S_PROPAG_VAR}</select>&nbsp;
			<!-- END show_propag -->
		</span></td>
	</tr>
	<!-- END row -->
	...
	<tr>
		<td align="center" class="<!-- BEGIN light -->row1<!-- BEGINELSE light -->row2<!-- END light -->" height="25" nowrap="nowrap"><span class="gensmall">
			<!-- BEGIN command -->
			<!-- BEGIN root_ELSE --><a href="{row.U_MOVE_UP}" title="{L_MOVE_UP}"><img src="{I_MOVE_UP}" border="0" alt="{L_MOVE_UP}" /></a>&nbsp;<!-- END root_ELSE -->
			<a href="{row.U_EDIT}" title="{L_EDIT}"><img src="{I_EDIT}" border="0" alt="{L_EDIT}" /></a>
			<a href="{row.U_CREATE}" title="{L_CREATE}"><img src="{I_CREATE}" border="0" alt="{L_CREATE}" /></a>
			<!-- BEGIN root_ELSE --><a href="{row.U_DELETE}" title="{L_DELETE}"><img src="{I_DELETE}" border="0" alt="{L_DELETE}" /></a>&nbsp;<!-- END root_ELSE -->
			<a href="{row.U_SYNCHRO}" title="{L_SYNCHRO}"><img src="{I_SYNCHRO}" border="0" alt="{L_SYNCHRO}" /></a>
			<!-- BEGIN root_ELSE --><a href="{row.U_MOVE_DOWN}" title="{L_MOVE_DOWN}"><img src="{I_MOVE_DOWN}" border="0" alt="{L_MOVE_DOWN}" /></a><!-- END root_ELSE -->
			<!-- END command -->
		</span></td>
	</tr>
	<!-- END row -->
