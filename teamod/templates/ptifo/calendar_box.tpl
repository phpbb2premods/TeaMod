...
<tr>
	...
	<tr>
		<td><a href="{U_PREC}" title="{L_PREC}"><img src="{I_PREC}" border="0" alt="{L_PREC}" hspace="4" /></a></td>
		<td width="100%" align="center"><span class="gensmall">
			<select name="month" onchange="this.form.submit();">{S_MONTH}</select><select name="year" onchange="this.form.submit();">{S_YEAR}</select>
			&nbsp;<select name="{POST_FORUM_URL}" onchange="this.form.submit();">{S_FORUM_LIST}</select>&nbsp;<input type="image" src="{I_GO}" alt="{L_GO}" align="top" />
		</span></td>
		<td><a href="{U_NEXT}" title="{L_NEXT}"><img src="{I_NEXT}" border="0" alt="{L_NEXT}" hspace="4" /></a></td>
	</tr>
	</table></td>
</tr>
<tr>
