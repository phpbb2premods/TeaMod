...
//-- add
			...
			$hypercell_class = ($this->data[$forum_id]['forum_status'] == FORUM_LOCKED) ? $hcc_lock : ($this->data[$forum_id]['sum_flag_unread'] ? $hcc_new : $hcc_dft);
...
//-- add
				...
				'HYPERCELL_ONCLICK' => ( $config->data['hypercell_onclick'] ) ? ' onclick="window.location.href=\'' . $config->url($this->requester, array(POST_FORUM_URL => $forum_id), true) . '\';"' : '',
//-- fin mod : hypercell class -------------------------------------------------
...
//-- mod : hypercell class -----------------------------------------------------
//-- add
					'HYPERCELL_LAST_ONCLICK' => ( empty($last_post_data['forum_last_post_id']) || !$config->data['hypercell_onclick'] ) ? '' : ' onclick="window.location.href=\'' . $config->url('viewtopic', array(POST_POST_URL => $last_post_data['forum_last_post_id']), true, $last_post_data['forum_last_post_id']) . '\';"',
//-- fin mod : hypercell class -------------------------------------------------
